...
| `temnion-studio` | Tauri 2 + React/TypeScript desktop client and interactive Web Workbench featuring Query Studio with real-time Live Subscription Streaming (CDC), Temporal Plane, Branches & Causality, Ingestion Console, Schema & Entity Catalog, Connections Manager, and Storage & Capabilities |
...
| `clients/python` (`temnion`) | Official zero-dependency Python client library for querying (SQL, TemQL, Compact Tem) and live CDC event streaming (`LiveEvent`) over TNP sockets |
| `clients/typescript` (`@temnion/client`) | Official TypeScript/Node.js client library with native socket transport, strong types, and `AsyncIterable` live CDC event streaming |
